...
( double s_min, double s_max, int s_bin_num, int mu_bin_num )
{
	const double ds = ( s_max - s_min )
		/ double ( s_bin_num - 1 );
	for( int i = 0; i < s_bin_num; ++ i )
		s_buf.push_back( i * ds + s_min );
	const double dmu = 1. / double( mu_bin_num );
	for( int i = 0; i < mu_bin_num + 1; ++ i )
		mu_buf.push_back( i * dmu );	
	return;
}
...
( const double & mu_min, const double & mu_max,
  const double & s )
{
	const double d_mu = mu_max - mu_min;

	intg.gl_clear(  );
	for( int i = 0; i < intg.gl_num; ++ i )
	{
		const double mu_s = mu_min + intg.gl_xi( i ) * d_mu;
		const double temp_integrand
			= inner_integration( s, mu_s );
		intg.gl_read( temp_integrand );		
	}
...
{	std::cout << "Generating xi_s: ";
	...
	std::vector<double> temp( mu_buf.size(  ) - 1, 0. );
	...
	for( unsigned i = 0; i < s_buf.size(  ); ++ i )
	{
		...
		for( unsigned j = 0; j < mu_buf.size(  ) - 1; ++ j )
		{
			const double mu_min = mu_buf[ j ];
			const double mu_max = mu_buf[ j + 1 ];
			temp[ j ] = wedge_integration( mu_min, mu_max, s );
		}
		xi_wedge_buf.push_back( temp );
	}
	return;
}
...
void xi_stream::output(  )
{
    ...
    std::string app_file_name = out_file_name + "_appendix";
    std::ofstream fout_app( app_file_name.c_str(  ) );
	std::string wedge_file_name = out_file_name + "_wedge";
	std::ofstream fout_wedge( wedge_file_name.c_str(  ) );

	for( unsigned j = 0; j < mu_buf.size(  ); ++ j )
		fout_wedge << mu_buf[ j ] << ' ';
